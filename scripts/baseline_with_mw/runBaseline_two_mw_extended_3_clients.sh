...
if check_if_machines_down ${all_machines[@]}; then
    ./startVms.sh aslvm1 aslvm2 slvm4 aslvm5 aslvm6
	if [ "$?" == "1" ]; then
...
create_output_dir $output_dir "client1" "client2" "client3" "mw1" "mw2" "server1"
...
for run in "${runs[@]}"; do
    for c in "${vc[@]}"; do
        for workers in "${nof_workers[@]}"; do
            start_time=$(date +%s)
            exp_params="#client=${c} #workers=${workers} run=${run} write-only then read-only"
            ...
            echo "starting mws again: ${mw_cmd}"
            ssh siegli@mw1 $mw_cmd_1 &
            disown #we don't want to wait for this job
            ssh siegli@mw2 $mw_cmd_2 &
            disown #we don't want to wait for this job
            ...
            run_memtier "client2" "mw1" 1 ${c} "0:1" "./${output_dir}/client2_i1_c${c}_ro_mwt${workers}_run${run}.log" ${time}
            run_memtier "client2" "mw2" 1 ${c} "0:1" "./${output_dir}/client2_i2_c${c}_ro_mwt${workers}_run${run}.log" ${time}
            run_memtier "client3" "mw1" 1 ${c} "0:1" "./${output_dir}/client3_i1_c${c}_ro_mwt${workers}_run${run}.log" ${time}
            run_memtier "client3" "mw2" 1 ${c} "0:1" "./${output_dir}/client3_i2_c${c}_ro_mwt${workers}_run${run}.log" ${time}
            wait_for_jobs
...
echo "-----------------------------------------------------------------------------------"
echo "done with baseline for two mw extended"
echo "-----------------------------------------------------------------------------------"
