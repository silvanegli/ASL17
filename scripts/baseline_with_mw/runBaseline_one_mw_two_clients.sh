...
time=62
output_dir="baseline_one_mw_two_clients_128workers_$(date +"%Y-%m-%d-%H:%M")" 
experiment_dir="/home/siegli/Code/asl-17/experiment_outputs"
local_result_dir="${experiment_dir}/${output_dir}"
all_machines=("client1" "client2" "mw1" "server1")

vc=(1 8 16 24 32 40 48 56 64)
runs=(1 2 3)
...
create_output_dir $output_dir "client1" "client2" "mw1" "server1"

from=("mw1" "client1" "client2")
to=("mw1" "server1")
...
#	        if [[ "$run" -eq 2 ]]; then
#				if [[ "$c" -lt 64 ]]; then
#					echo "continue as experiment run: ${run}, clients: ${clients}, workers: ${workers} was already contucted"
#					continue
#				fi
#			fi
...
echo "-----------------------------------------------------------------------------------"
echo "done with baseline for one mw two clients"
echo "-----------------------------------------------------------------------------------"

experiment_dir="/home/siegli/Code/asl-17/experiment_outputs"
local_result_dir="${experiment_dir}/${output_dir}"

get_experiment_logs $local_result_dir "./${output_dir}/*" client1 client2 mw1 server1
