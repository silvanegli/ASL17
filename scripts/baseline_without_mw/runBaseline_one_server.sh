...
#----------------------------------------------------------
output_dir="baseline_one_server_$(date +"%Y-%m-%d-%H:%M")"
all_machines=("client1" "client2" "client3" "server1")
from=("client1" "client2" "client3")
to=("server1")

if check_if_machines_down ${all_machines[@]}; then
	./startVms.sh aslvm1 aslvm2 aslvm3 aslvm6
    if [ "$?" == "1" ]; then
...
for run in "${runs[@]}"; do
    for c in "${vc[@]}"; do
        ...
        run_memtier "client1" "server1" 2 ${c} "1:0" "./${output_dir}/client1_i1_c${c}_wo_r${run}.log" ${time}
        pidCl1=$!
        run_memtier "client2" "server1" 2 ${c} "1:0" "./${output_dir}/client2_i1_c${c}_wo_r${run}.log" ${time}
        pidCl2=$!
        run_memtier "client3" "server1" 2 ${c} "1:0" "./${output_dir}/client3_i1_c${c}_wo_r${run}.log" ${time}
    	pidCl3=$!
        wait $pidCl1
	    wait $pidCl2	    
        wait $pidCl3
        #kill_and_wait_for_process_shutdown "dstat" ${all_machines[@]}
        ...
        run_memtier "client1" "server1" 2 ${c} "0:1" "./${output_dir}/client1_i1_c${c}_ro_r${run}.log" ${time}
        pidCl1=$!
        run_memtier "client2" "server1" 2 ${c} "0:1" "${output_dir}/client2_i1_c${c}_ro_r${run}.log" ${time}
        pidCl2=$!
        run_memtier "client3" "server1" 2 ${c} "0:1" "${output_dir}/client3_i1_c${c}_ro_r${run}.log" ${time}
        pidCl3=$!
        wait $pidCl1
        wait $pidCl2        
        wait $pidCl3
