...
nof_clients_per_thread = [1, 4, 12, 20, 28, 32, 40, 56, 64]
nof_clients_per_thread = [1, 4, 8, 12, 16, 20, 24, 28, 32, 40, 48]

nof_clients = [c * (upper_instance_id - lower_instance_id + 1) * (upper_machine_id - lower_machine_id + 1) for c in nof_clients_per_thread]
...
def plot_memtier_results(experiment_folder):
        ...
        for run in range(lower_run_id, upper_run_id + 1):
            ...
            for machine in range(lower_machine_id, upper_machine_id + 1):
                for instance in range (lower_instance_id, upper_instance_id + 1):
                    wo_parse = parse_memtier_single_run(experiment_folder, machine, instance, clients, run, 'wo').clean_trace(nof_memtier_startups, nof_memtier_cooldowns)
                    ro_parse = parse_memtier_single_run(experiment_folder, machine, instance, clients, run, 'ro').clean_trace(nof_memtier_startups, nof_memtier_cooldowns)
    ...
    plotter.figure(1)
    plotter.errorbar(nof_clients, [res.total_avg_tps for res in avg_wo_results], yerr=[res.err_tps for res in avg_wo_results],
                     fmt='ro-', label="write-only requests", capsize=3)
    plotter.errorbar(nof_clients, [res.total_avg_tps for res in avg_ro_results], yerr=[res.err_tps for res in avg_ro_results],
   ...
   # plotter.plot(nof_clients, [1000 * clients / res.total_avg_st for res, clients in zip(avg_ro_results, nof_clients)], linestyle='-', color="lightgreen", marker='o', label="r-o interactive law")
    ...
    print_data("write-only data", nof_clients, avg_wo_results, "baseline_without_mw/write-only-one-client")
    print_data("read-only data", nof_clients, avg_ro_results, "baseline_without_mw/read-only-one-client")
